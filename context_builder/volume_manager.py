...
class VolumeManager:
    ...
    def add_callers(self, category_list, callers_dict, category_label, confidence="HIGH", distance=0):
    ...
    def flush_all_volumes(self):
        ...
        def try_append(section_header, items, format_fn):
            ...
            for item in items:
                ...
                if payload_bytes + section_bytes + header_cost + item_bytes > self.max_bytes:
                    truncated = True
                    break
        ...
        def format_object(obj):
            ...
            return f"### `{obj['file']}` -> `{obj['function_name']}()`\n```{lang}\n{obj['source_block']}\n```\n"
