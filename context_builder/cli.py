...
from .lsp_client import cleanup_zombie_lsps, get_lsp_references
...
def _process_single_caller_reference(
    ref_path, occurrences, file_cache, processed_spans, queue, depth
):
    ...
    for occ in occurrences:
        ...
        if occ_line <= 0:
            continue
        start, end = extract_function_bounds(
            ref_path, occ_line, file_cache=file_cache
        )
        if start is None:
            continue
...
def _resolve_references(curr_file, curr_line, curr_func, args, file_cache, all_repo_files):
    ...
    callers = get_lsp_references(
        ...
        curr_func,
        args.lsp_timeout,
        args.max_interface_depth,
        args.disable_pruning,
        file_cache=file_cache,
    )
    if callers is None:
        ...
        if AST_ENGINE.is_supported(ext):
            callers = trace_lexical_dependencies_ast(
                curr_func, all_repo_files, file_cache=file_cache
            )
        else:
...
def _merge_macro_and_build_linkages(
    curr_file, curr_func, ext, callers, cpp_linkages, all_repo_files, file_cache, args
):
...
def _process_caller_depth_step(
    curr_file, curr_line, curr_func, depth, processed_spans, all_repo_files,
    file_cache, ffi_exports, cpp_linkages, vm, queue, args
):
    """Trace callers for a single queue item at the current depth step."""
    callers = _resolve_references(
        curr_file, curr_line, curr_func, args, file_cache, all_repo_files
    )
    ext = os.path.splitext(curr_file)[1]
    _merge_macro_and_build_linkages(
        curr_file, curr_func, ext, callers, cpp_linkages,
        all_repo_files, file_cache, args
    )
    ...
    for ref_path, occurrences in filtered_callers.items():
        _process_single_caller_reference(
            ref_path, occurrences, file_cache, processed_spans, queue, depth
        )


def _traverse_callers_bfs(
    queue, processed_spans, all_repo_files, file_cache, ffi_exports,
    cpp_linkages, vm, args
):
    ...
    while queue:
        ...
        if depth < args.caller_depth:
            _process_caller_depth_step(
                curr_file, curr_line, curr_func, depth, processed_spans,
                all_repo_files, file_cache, ffi_exports, cpp_linkages, vm,
                queue, args
            )


def _process_single_callee(
    callee_name, depth, all_repo_files, file_cache,
    processed_callee_spans, vm, callee_queue, args
):
    """Resolve, bounds-check, and semantically split a single called function."""
    def_file, def_line = find_callee_definition(
        callee_name, all_repo_files, file_cache=file_cache
    )
    if not def_file or not def_line:
        return
    def_start, def_end = extract_function_bounds(
        def_file, def_line, file_cache=file_cache
    )
    ...
    subunits = split_massive_block_ast(
        func_chunk, def_file, max(1, args.max_lines - 100)
    )
    for sub in subunits:
...
def _traverse_callees_bfs(callee_queue, processed_spans, all_repo_files, file_cache, vm, args):
...
def run_scan(args, start_ref=None, end_ref=None, output_dir=".", repo_root=None):
    ...
    _traverse_callers_bfs(
        queue, processed_spans, all_repo_files, file_cache, ffi_exports,
        cpp_linkages, vm, args
    )

    _traverse_callees_bfs(
        callee_queue, processed_spans, all_repo_files, file_cache, vm, args
    )
